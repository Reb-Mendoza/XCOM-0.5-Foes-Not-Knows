...
function startGame(map) {
    ...
    for (i=0; i < wall.vertical.X.length; i++) {
        ...
        entityEl.setAttribute("position", {x: (wall.vertical.X[i] * -2)+2, y: -4.5, z: (wall.vertical.Y[i] * 2)});
    ...
    for (i=0; i < wall.horizontal.X.length; i++) {
        ...
        sceneEl.append(entityEl);
        entityEl.setAttribute("mixin", "hardWall");
        entityEl.setAttribute("position", {x: (wall.horizontal.X[i] * -2)+1, y: -4.5, z: (wall.horizontal.Y[i] * 2)-1});
